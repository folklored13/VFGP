...
def create_toolbox(n_features):
    ...
    # 添加一个常数叶子节点（Ephemeral Constant），其值在每次生成时从[-1, 1]均匀采样
    # pset.addEphemeralConstant("ERC", lambda: random.uniform(-1.0, 1.0))
    erc_generator = partial(random.uniform, -1.0, 1.0)
    pset.addEphemeralConstant("ERC", erc_generator)
