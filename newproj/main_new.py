...
from sklearn.metrics import f1_score, accuracy_score # 主要用 F1

# 导入自定义模块
import config # config.py 保持不变，或者可以更新 NUM_JOBS 和 N_SPLITS
from gp_setup import create_toolbox
from evaluation import evalVFGP, transform_data # evaluation.py 基本保持不变
    ...
    for job_idx in range(num_total_jobs):
        ...
        for run_idx in range(num_runs_per_job): #对应日志中的 "Fold"
            ...
            pop, logbook = algorithms.eaMuPlusLambda(
                ...
                stats=None, halloffame=hof, verbose=False # 可以暂时关闭stats，模仿ECJ输出
            )
