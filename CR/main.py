...
import numpy as np
import os
from deap import base, creator, tools, gp, algorithms # Import algorithms
from deap.gp import PrimitiveSetTyped
# import evalGP_fgp as evalGP # Remove custom evalGP
import functions as fs # Renamed import alias
...
import math # Import math for sin/cos scalar functions

# Assume split_ph2.py has already been run to generate these files
...
# Define Terminal: Access a specific feature (index i) from the input vector
def get_feature(index):
    # This function returns a callable that, when called with the input vector,
    # returns the float value at the given index.
    # DEAP automatically handles the wiring when the tree is compiled.
    def feature_access_func(features):
        return float(features[index]) # Ensure it returns a float
    return feature_access_func
...
for i in range(x_train.shape[1]): # x_train.shape[1] is the total number of original features (652)
...
def evalVFGP(individual):
    try:
        ...
        selected_indices = sorted(list({int(node.name[1:]) for node in individual if isinstance(node, gp.Terminal)}))
...
if __name__ == "__main__":
    ...
    # selBest selects k best from the population it's given.
    # eaMuPlusLambda selects mu survivors from the combined mu parents + lambda offspring
    # Using selBest as the survivor selector effectively implements elitism if k=mu.
    # To implement elitism_prob specifically, you might need to manually select elites
    # and combine them with survivors selected by other means, or use a different algorithm structure.
    # Let's use selBest for survivors as the paper lists Elitism Rate.
    # However, the paper lists Tournament selection for 'Selection type', which might apply to survivors too.
    # The most standard way to incorporate elitism in DEAP with Tournament is often
    # selecting pop_size-elites via tournament and adding elites from the prev generation.
    # Let's stick closer to the structure implied by the custom eaSimple, which selected parents, varied, then added elites back.
    # Replicating that with standard DEAP requires a loop or a custom algorithm.
    # Or, simplify: Use eaSimple with Tournament selection. HallOfFame tracks the best overall.
    # Let's use standard eaSimple and rely on HallOfFame. Elitism rate might be more about
    # encouraging preservation of good individuals via tournament selection pressure and HallOfFame.
    # Alternatively, use eaMuPlusLambda with tournament for parents and tournament for survivors,
    # and manually preserve the top elite_num individuals from the hof.

    # Let's try eaMuPlusLambda as it allows explicit survivor selection (often selBest)
    ...
    final_selected_indices = sorted(list({int(node.name[1:]) for node in best_individual if isinstance(node, gp.Terminal)}))
